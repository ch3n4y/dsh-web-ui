  ...
  create(payload: HostPayload): SshHostEntry {
    ...
    const entry: SshHostEntry = {
      ...
      auth: {
        ...
        agentPath: payload.auth.kind === 'agent' ? payload.auth.agentPath?.trim() || undefined : undefined,
      },
    ...
    if (patch.auth !== undefined) {
      ...
      entry.auth = {
        ...
        agentPath: auth.kind === 'agent'
          ? (auth.agentPath !== undefined ? auth.agentPath.trim() || undefined : entry.auth.agentPath)
          : undefined,
      }
    }
